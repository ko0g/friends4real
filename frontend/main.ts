...
const API_KEY_MAP: string = "1bc282243dca48c0acd5582d97d4e00a";

function get_start_point_icon(color: string): string {
    return `
...
function get_friend_point_icon(color: string): string {
    return `
...
window.onload = () => {
    const map = new Map({
        ...
        view: new View({
            center: fromLonLat([37.60, 55.65]),
            zoom: 15,
            maxZoom: 20,
            minZoom: 8
        })
    });
    const mainSource = new VectorSource();
    ...
    map.getViewport().addEventListener('pointermove', (e) => {
        if (mode_opt_route && following) {
            ...
            const lonLat = toLonLat(mapCoordinate);  // в долготу/широту

            startbtn!.innerHTML = `${lonLat[0].toFixed(3)}, ${lonLat[1].toFixed(3)}`;
        }
    });

    function append_icon(_scale: number, color: string, type: number, e, ind: number) {
        let svg: string;
        if (type == 0){
    ...
    function add_house(x: string, y: string, col: string, ord: number, ind: number, flag: boolean) {
    ...
    function get_route(fromPoint: string[], toPoint: string[], curColor: string, i: number, order: number) {
        const body = {
        ...
        .then(data => {
            ...
            const route = new LineString(data.coords).transform('EPSG:4326', 'EPSG:3857');
            const feature = new Feature({ geometry: route });
            feature.setStyle(new Style({
                ...
                text: new Text({
                    ...
                    fill: new Fill({ color: '#000' }),
                    stroke: new Stroke({ color: curColor, width: 4 }),
                    overflow: true
                })
            }));
    ...
    function calc_opt_order(start: string[], points: string[][]) {
    ...
    function calc_opt_point(points: string[][]) {
        ...
        .then(data => {
            ...
            start_point_defined = true;
            let color = 'green';
            append_icon(0.9, color, 0, { coordinate: fromLonLat(center) }, 0);
            return routes;
        })
        .catch(err => {
    ...
    function draw_routes(routes: [string[], string[], number][]) {
        while (list!.firstChild) {
            list!.removeChild(list!.firstChild);
        }
